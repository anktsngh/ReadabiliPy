...
// We want to load Readability-readerable, which isn't set up as commonjs libraries,
// and so we need to do some hocus-pocus with 'vm' to import them on a separate scope
// (identical) scope context.
var vm = require("vm");
var readabilityReaderablePath = path.join(__dirname, "Readability-readerable.js");
...
vm.runInNewContext(fs.readFileSync(readabilityReaderablePath), scopeContext, readabilityReaderablePath);

// var readabilityReaderable = scopeContext;
...
if (typeof(argv['o']) !== 'undefined') {
    ...
    outFilePath = inFilePath + ".readerable.json";
}
var html = readFile(inFilePath);

var doc = new scopeContext.JSDOM(html).window.document;
var readerable_bool = scopeContext.isProbablyReaderable(doc);

writeFile(JSON.stringify(readerable_bool), outFilePath);